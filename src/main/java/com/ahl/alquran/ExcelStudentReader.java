...
public class ExcelStudentReader {
    ...
    public List<StudentResponseDTO> readStudentsFromExcel(String filePath) throws IOException {
        ...
        try (FileInputStream fis = new FileInputStream(filePath);
            ...
            for (Row row : sheet) {
                ...
                try {
                    StudentResponseDTO studentDTO = createStudentFromRow(row, dataFormatter);
                    if (studentDTO != null) {
    ...
    private StudentResponseDTO createStudentFromRow(Row row, DataFormatter dataFormatter) {
        ...
        return StudentResponseDTO.builder().name(name)
                .level(level)
                .result(score*2)
                .year(1445).build();
    }
